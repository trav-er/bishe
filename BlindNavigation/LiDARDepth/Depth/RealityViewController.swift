...
public class RealityViewController: UIViewController,
    ...
    internal struct State {
        var showMesh: Bool = false                  //是否显示网格
        var showDistance: Bool = false              //是否显示距离
        ...
        var anchorEntityForMinDistanceFromCamera: () -> AnchorEntity = { AnchorEntity() }           //设置锚点模型
        //根据距离显示不同的线条颜色
        var colorForDistance: (Float) -> UIColor = { distance in
            ...
            default: return UIColor.systemIndigo //最大距离为5米
            }
        }
    }
    ...
    private func updateRaycast() {
        ...
        let raycastDistance = distance(resultWorldPosition, cameraTransform.translation)            //距离计算
